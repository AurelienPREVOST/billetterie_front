...
const EditProduct = (props) => {
  ...
  const verifyAddress = () => {
    ...
    axios
      .get(`https://api-adresse.data.gouv.fr/search/?q=${formattedAddress}`)
      .then((response) => {
  ...
  useEffect(() => {
    takeOneProduct(props.params.id)
      .then((res) => {
        ...
        setRankWidth(res.result.rankWidth)
      })
      .catch((err) => console.log(err));
  }, []);
      ...
      <form className="b-form" onSubmit={onSubmitForm}>
        ...
        <input
          ...
          onChange={(e) => {
            ...
            if (file) {
              // Vérifie que la taille du fichier fait moins de 1mo
        ...
        />
        {addressVerified ? (
          <p style={{ color: 'green' }}>Adresse trouvée</p>
        ) : (
          <p style={{ color: 'red' }}>Précisez mieux l'adresse</p>
        )}
